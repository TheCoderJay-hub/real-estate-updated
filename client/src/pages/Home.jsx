...
export default function Home() {
  const { data: offerListings = [] } = useQuery({
    queryKey: ['listings', { offer: true, limit: 4 }],
    queryFn: async () => {
  ...
  const { data: rentListings = [] } = useQuery({
    queryKey: ['listings', { type: 'rent', limit: 4 }],
    queryFn: async () => {
  ...
  const { data: saleListings = [] } = useQuery({
    queryKey: ['listings', { type: 'sale', limit: 4 }],
    queryFn: async () => {
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 grid md:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-in space-y-8">
            ...
            <p className="text-slate-600 text-lg md:text-xl max-w-lg leading-relaxed">
              Estate Connect helps you discover the most luxurious and comfortable properties across the country. Your perfect sanctuary is just a search away.
            </p>
