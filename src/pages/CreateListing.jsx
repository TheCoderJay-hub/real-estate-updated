...
export default function CreateListing() {
  ...
  const [files, setFiles] = useState([]); // Selected files from input
  ...
  const handleImageSubmit = (e) => {
    ...
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          console.error("Critical: Promise.all failed in handleImageSubmit:", err);
          setImageUploadError(err.message || "Image upload failed. Check the console for details.");
          setUploading(false);
        });
    } else {
    ...
    <div className="bg-beige-primary min-h-screen py-12 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-black text-slate-800 tracking-tight">Create <span className="gradient-text">New Listing</span></h1>
          <p className="text-slate-500 font-medium mt-2">Scale your reach and find the perfect buyer/tenant</p>
        </div>
        ...
        <form onSubmit={handleFormSubmit} className="grid lg:grid-cols-2 gap-12 animate-fade-in">
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4 pt-8">
              ...
              <p className="text-center text-slate-400 text-[10px] font-bold uppercase tracking-widest px-12">By publishing, you agree to our terms of service and property listing guidelines.</p>
            </div>
          </div>
        </form>
      </div>
